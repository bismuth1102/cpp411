 ...
 * technique
 */
void gauss(matrix_t &A, vector_t &B, vector_t &X, int grainsize) {
    // iterate over rows
    for (unsigned i = 0; i < A.getSize(); ++i) {
        ...
        big = parallel_reduce(blocked_range<int>(i, A.getSize(), grainsize), big, [&](const blocked_range<int>& r, int max)->double{
            for(auto k = r.begin()+1; k!=r.end(); k++){
                if (abs(A[k][i]) > max) {
                    max = abs(A[k][i]);
                    row = k;
                }
        ...
        // column
        parallel_for(blocked_range<int>(i+1,A.getSize(),grainsize),
...
//    bool parallel = false; // use parallelism?
    ...
    int o;
    while ((o = getopt(argc, argv, "t:n:g:")) != -1) {
        switch (o) {
    ...
    std::cout << "n,g,t = " << size << ", " << grainsize << ", " << threads << std::endl;
