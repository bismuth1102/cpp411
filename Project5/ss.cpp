...
void handle_get(http_request message) {
    ...
    string uri = http::uri::decode(message.relative_uri().path());
    if(uri=="/"){
        streams::basic_istream<uint8_t> fistream = OPENSTR_R<uint8_t>(U("index.html")).get();

        http_response response(200);
        response.set_body(fistream);

        message.reply(response, "text/html");
    }
    else if(uri=="/file"){
        string filename = http::uri::decode(message.relative_uri().query());
        streams::basic_istream<uint8_t> fistream = OPENSTR_R<uint8_t>(U(filename)).get();
        ...
        message.reply(200, result);
    }
    else{
        message.reply(200, "wrong url");
    }
